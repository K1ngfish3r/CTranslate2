...
int main(int argc, char* argv[]) {
  ...
  cmd_options.add_options("Device")
    ...
    ("device", "Device to use (can be cpu, cuda, xpu, auto).",
  ...
  cmd_options.add_options("Model")
    ...
    ("cpu_compute_type", "Computation type on CPU devices (overrides compute_type)",
     cxxopts::value<std::string>())
    ("xpu_compute_type", "Computation type on XPU devices (overrides compute_type)",
     cxxopts::value<std::string>())
    ;
  ...
  case ctranslate2::Device::CUDA:
    ...
    break;
  case ctranslate2::Device::XPU:
    if (args.count("xpu_compute_type"))
      compute_type = ctranslate2::str_to_compute_type(args["xpu_compute_type"].as<std::string>());
    break;
  };
